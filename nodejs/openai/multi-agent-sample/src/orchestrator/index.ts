...
initializeObservability('Multi-Agent Orchestrator');
...
console.log(`[Orchestrator] Environment: NODE_ENV=${process.env.NODE_ENV}, isDevelopment=${isDevelopment}`);
...
const port = Number(process.env.ORCHESTRATOR_PORT) || 3978;
const host = process.env.AGENT_HOST ?? (isDevelopment ? 'localhost' : '0.0.0.0');
server.listen(port, host, () => {
  console.log(`[Orchestrator] listening on ${host}:${port} for appId ${authConfig.clientId}`);
