...
initializeObservability('Multi-Agent Reviewer');
...
console.log(`[Reviewer] Environment: NODE_ENV=${process.env.NODE_ENV}, isDevelopment=${isDevelopment}`);
...
server.get('/api/health', (_req, res: Response) => {
  res.status(200).json({ status: 'healthy', service: 'reviewer', timestamp: new Date().toISOString() });
});
...
server.post('/api/run', async (req: express.Request, res: Response) => {
  try {
    const result = await handleReviewerRequest(req.body, req.headers);
...
const port = Number(process.env.REVIEWER_PORT) || 4003;
const host = process.env.AGENT_HOST ?? (isDevelopment ? 'localhost' : '0.0.0.0');
server.listen(port, host, () => {
  console.log(`[Reviewer] listening on ${host}:${port} for appId ${authConfig.clientId}`);
}).on('error', (err: unknown) => {
  console.error(err);
  process.exit(1);
}).on('close', () => {
  console.log('[Reviewer] Server closed');
  process.exit(0);
});
