# Autonomous Agent — Node.js Sample
...
| Microsoft OpenTelemetry distro with S2S exporter | `src/index.ts` |
...
Local dev (client secret):
  Blueprint ClientSecret + fmi_path(agentId) → Entra token endpoint
...
> **Note:** The local dev path uses a raw HTTP token request instead of MSAL because `@azure/msal-node` does not yet support the `fmi_path` parameter. The production MSI path uses MSAL with `fmiPath` via type assertion.
