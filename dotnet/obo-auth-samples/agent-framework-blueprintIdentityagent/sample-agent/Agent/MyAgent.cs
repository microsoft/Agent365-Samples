...
namespace Agent365AgentFrameworkSampleAgent.Agent
{
    public class MyAgent : AgentApplication
    {
        ...
        private readonly BlueprintFmiTokenProvider? _blueprintFmiTokenProvider = null;
        ...
        /// <summary>
        /// Checks the current HTTP request's Authorization header for a Bearer token
        /// that was forwarded by the /api/obo-messages endpoint (stored in HttpContext.Items).
        /// </summary>
        private bool TryGetUserAssertionFromHttpContext(out string? userAssertionToken)
        {
            userAssertionToken = _httpContextAccessor?.HttpContext?.Items["UserAssertionToken"] as string;
            return !string.IsNullOrEmpty(userAssertionToken);
        }
        ...
        private const string McpGatewayDefaultScope = "ea9ffc3e-8a23-4a7d-836d-234d7c7565c1/.default";
            ...
            ILogger<MyAgent> logger) : base(options)
        {
            ...
            _fmiTokenProvider = connections.GetConnection("BotConnection");
            _blueprintFmiTokenProvider = _fmiTokenProvider as BlueprintFmiTokenProvider;
        ...
        private async Task<AIAgent?> GetClientAgent(ITurnContext context, ITurnState turnState, IMcpToolRegistrationService? toolService, string? authHandlerName)
        {
            ...
            else if (TryGetUserAssertionFromHttpContext(out var userAssertion) && _blueprintFmiTokenProvider != null)
            {
                _logger?.LogInformation("OBO flow: found user assertion token from Authorization header. Performing OBO exchange.");
                accessToken = await _blueprintFmiTokenProvider.GetOnBehalfOfAccessTokenAsync(
                    [McpGatewayDefaultScope], userAssertion!);
                agentId = Utility.ResolveAgentIdentity(context, accessToken);
                _logger?.LogInformation("OBO flow: resolved agentId='{AgentId}'", agentId ?? "(null)");
            }
            else
            {
