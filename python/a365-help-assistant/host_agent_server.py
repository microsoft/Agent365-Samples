...
class GenericAgentHost:
    ...
    def _setup_handlers(self):
        ...
        async def help_handler(context: TurnContext, _: TurnState):
            ...
            welcome_message = (
                "👋 **Welcome to A365 Help Assistant!**\n\n"
            ...
            try:
                ...
                with BaggageBuilder().tenant_id(tenant_id).agent_id(agent_id).build():
                    ...
                    if not self.agent_instance:
                        error_msg = "❌ Sorry, the A365 Help Assistant is not available."
    ...
    def create_auth_configuration(self) -> AgentAuthConfiguration | None:
        ...
        if environ.get("BEARER_TOKEN"):
            logger.info("🔑 BEARER_TOKEN present - will use for MCP server authentication")
        else:
    ...
    def start_server(self, auth_configuration: AgentAuthConfiguration | None = None):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            if s.connect_ex(("127.0.0.1", desired_port)) == 0:
                logger.warning(f"⚠️ Port {desired_port} already in use. Attempting {desired_port + 1}.")
                port = desired_port + 1

        print("=" * 80)
        print("🤖 A365 Help Assistant - Helpdesk Agent for Microsoft Agent 365")
        print("=" * 80)
        print(f"\n🔒 Authentication: {'Enabled' if auth_configuration else 'Anonymous'}")
        print("📚 Using Microsoft Agents SDK & Agent 365 SDK")
        print("🔍 Documentation search enabled")
        ...
        print("🎯 Ready to help with Agent 365 questions!\n")
