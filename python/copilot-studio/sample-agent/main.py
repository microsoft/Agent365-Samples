...
"""
Copilot Studio Sample Agent – Server Entry Point
...
from microsoft.opentelemetry.a365.hosting.token_cache_helpers import (  # noqa: E402
    AgenticTokenCache,
    AgenticTokenStruct,
)
from microsoft.opentelemetry.a365.runtime import (  # noqa: E402
...
# Observability / exporter — DEBUG to see export HTTP responses
logging.getLogger("microsoft_agents_a365.observability").setLevel(logging.DEBUG)
...
# ---------------------------------------------------------------------------
# Observability token cache (docs: "Agentic token cache with Agent Framework apps")
# ---------------------------------------------------------------------------
# The token is acquired at runtime from TurnContext (incoming message) — NOT
# from static env vars.  The sync resolver is called by the batch exporter
# thread; the async handler refreshes the cache on each turn.
_token_cache = AgenticTokenCache()
_cached_tokens: dict[tuple[str, str], str | None] = {}


def _sync_token_resolver(agent_id: str, tenant_id: str) -> str | None:
    """Sync resolver called by the OpenTelemetry exporter thread."""
...
class CopilotStudioAgentHost:
    """Hosts the Copilot Studio sample agent."""
    ...
    async def _setup_observability_token(
        ...
        """Register observability token from the TurnContext (incoming message).

        Uses the AgenticTokenCache pattern from the docs — the token is
        acquired at runtime via OBO auth, not from static env vars.
        """
        if not self.auth_handler_name:
            logger.debug("No auth handler — skipping observability token registration")
            return

        try:
            _token_cache.register_observability(
                agent_id=agent_id,
                tenant_id=tenant_id,
                token_generator=AgenticTokenStruct(
                    authorization=self.agent_app.auth,
                    turn_context=context,
                ),
                observability_scopes=get_observability_authentication_scope(),
            )
            token = await _token_cache.get_observability_token(agent_id, tenant_id)
            _cached_tokens[(agent_id, tenant_id)] = token.token if token else None
            logger.debug(
                "Observability token refreshed for agent_id=%s, tenant_id=%s",
                agent_id, tenant_id,
            )
        except Exception:
            logger.warning("Failed to refresh observability token", exc_info=True)

    async def _validate_agent_and_setup_context(self, context: TurnContext):
        """Validate agent availability and set up observability context."""
        ...
        logger.info(
            ...
            agent_id,
            tenant_id,
            "activity.recipient" if getattr(recipient, "agentic_app_id", None) else "env",
        )
            ...
            try:
                ...
                with BaggageBuilder().tenant_id(tenant_id).agent_id(agent_id).build():
                    ...
                    if not user_message.strip():
                        await context.send_activity(
                            "Please send me a message and I'll forward it to Copilot Studio!"
                        )
                        return
    ...
    def start_server(self, auth_configuration: AgentAuthConfiguration | None = None) -> None:
        ...
        print("=" * 80)
        print("Copilot Studio Sample Agent (Python)")
        print("=" * 80)
...
def main() -> None:
    ...
    if environ.get("ENABLE_OBSERVABILITY", "true").lower() == "true":
        # Use the Microsoft OpenTelemetry Distro with AgenticTokenCache.
        # The token resolver is sync (called from the exporter thread);
        # the cache is refreshed async in each message handler turn.
        use_microsoft_opentelemetry(
            enable_a365=True,
            enable_azure_monitor=False,
            a365_token_resolver=_sync_token_resolver,
        )
        logger.info(
            "Observability configured via Microsoft OpenTelemetry Distro "
            "(enable_a365=True, token_resolver=AgenticTokenCache, a365_exporter=%s)",
            environ.get("ENABLE_A365_OBSERVABILITY_EXPORTER", "false"),
        )
    else:
        logger.info("Observability disabled (ENABLE_OBSERVABILITY=false)")

    host = CopilotStudioAgentHost()
