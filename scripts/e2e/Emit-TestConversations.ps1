...
foreach ($conv in $results.Conversations) {
    $statusIcon = if ($conv.Passed) { "✅" } else { "❌" }
...
if ($env:GITHUB_STEP_SUMMARY) {
    ...
    $passedIcon = if ($results.FailedTests -eq 0) { "✅" } else { "⚠️" }
...
"@
    ...
    foreach ($conv in $results.Conversations) {
        $statusIcon = if ($conv.Passed) { "✅ Pass" } else { "❌ Fail" }
...
<details>
<summary>📝 Full Test Details (click to expand)</summary>
...
if ($results.FailedTests -gt 0) {
    Write-Host ""
    Write-Host "⚠️ $($results.FailedTests) test(s) failed" -ForegroundColor Yellow
}
else {
    Write-Host ""
