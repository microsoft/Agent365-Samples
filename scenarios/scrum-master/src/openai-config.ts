...
export function configureOpenAIClient(): void {
  if (isAzureOpenAI()) {
    ...
    // IMPORTANT: @openai/agents built-in tracing exporter instantiates a plain OpenAI client
    // that requires OPENAI_API_KEY. When using Azure OpenAI we don't have that key, and we
    // already ship traces via a365Observability, so disable the built-in tracer.
    setTracingDisabled(true);
    console.log('[OpenAI Config] @openai/agents built-in tracing disabled (using a365Observability instead)');
