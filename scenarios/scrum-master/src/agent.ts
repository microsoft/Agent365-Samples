...
import { tryHandleCommand } from './handlers/commands';
import { handleStandupSubmit } from './handlers/standup';
import { handleAnswer } from './handlers/answer';
import { handleReconcileSubmit } from './handlers/reconcile';
import { handleBlockerSubmit, handleMeetingSubmit } from './handlers/chase';
import { upsertConversationReference } from './services/team-roster';
 ...
 */
  async handleAgentMessageActivity(turnContext: TurnContext, state: TurnState): Promise<void> {
    ...
    // SMA: capture / refresh the conversation reference so proactive DMs work later.
    // Best-effort — SharePoint may not be provisioned yet during Phase 0 setup.
    try {
      await upsertConversationReference(turnContext);
    } catch (e) {
      console.warn('[SMA] upsertConversationReference failed (non-fatal):', (e as Error).message);
    }

    // SMA: Adaptive Card submits arrive as Message activities with `activity.value` populated.
    const value = turnContext.activity.value as { action?: string } | undefined;
    if (value?.action?.startsWith('standup.')) {
      await handleStandupSubmit(turnContext);
      return;
    }
    if (value?.action?.startsWith('reconcile.')) {
      await handleReconcileSubmit(turnContext);
      return;
    }
    if (value?.action?.startsWith('blocker.')) {
      await handleBlockerSubmit(turnContext);
      return;
    }
    if (value?.action?.startsWith('meeting.')) {
      await handleMeetingSubmit(turnContext);
      return;
    }

    // SMA: slash commands short-circuit the LLM.
    if (await tryHandleCommand(turnContext)) {
      return;
    }
    ...
    // SMA: free-text messages go to the Answer handler (MVP 5) — grounded Q&A over
    // the live Jira board via function-calling. Falls back to the generic LLM below
    // if SMA_ANSWER_DISABLED=true (kept as an escape hatch for the base sample demo).
    if (process.env.SMA_ANSWER_DISABLED !== 'true') {
      await handleAnswer(turnContext, userMessage);
      return;
    }
