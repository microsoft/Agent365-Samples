...
points for the `cos-agent`. Intended audience: developers reading the code
...
sequenceDiagram
  ...
  loop every POLL_TASKS_MS
    ...
    TC->>Leader: "Peyton completed X" (or "Blocker resolved — …")
  end
```
...
├── manifest/
│   ├── manifest.json                         # Teams app manifest (Agent 365 shape)
│   └── agenticUserTemplateManifest.json      # per-user template
├── ToolingManifest.json                      # MCP server declarations (Teams / Mail / Calendar)
├── .env / .env.template                      # local config
├── a365.config.json / a365.generated.config.json
...
| `LEADER_NAME` | Optional — used in card copy (e.g. "Assigned by: Mario") |
